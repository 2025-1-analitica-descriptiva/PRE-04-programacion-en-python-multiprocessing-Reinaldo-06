...
def reducer(sequence):
    ...
    def chunkify(sequence, num_chunks):
        return [sequence[i::num_chunks] for i in range(num_chunks)] # Separar la secuencia en partes iguales

    def merge_results(chunks):
        final_result = defaultdict(int) # Usar defaultdict para evitar KeyError
        for chunk in chunks:
            for key, value in chunk: # Iterar sobre cada par (key, value) en el chunk
                final_result[key] += value
        return list(final_result.items()) # Convertir el resultado a una lista de tuplas

    num_chunks = os.cpu_count()
    chunks = chunkify(sequence, num_chunks) # Dividir la secuencia en partes iguales

    with Pool(num_chunks) as pool: # Crear un pool de procesos
        chunk_results = pool.map(sum_by_key, chunks) # Aplicar la función sum_by_key a cada chunk
